	...
	if(isset($_POST['save']))
	{
		...
		if($result2)
		{
			...
			while(true)
			{
				...
				if ($result=mysqli_query($conn, "insert into `order`(OrderId,CreatedTime,AcceptedTime,status,AdminUserDetailId) values(".$oid.",'".$today."','".$today."',3,".$_SESSION['admin'].")"))
				if($result){
					while ($ids != "") {
...
<?php $db=new _database();
		$conn = $db->connect();
		//echo "select o.OrderId,MIN(CreatedTime),SUM(Quantity),SUM(Quantity*Price) from `order` o, orderitem i where o.OrderId=i.OrderId and o.tableid is NULL group by o.OrderId";
		$result2=mysqli_query($conn, "select o.OrderId,MIN(CreatedTime),SUM(Quantity),SUM(Quantity*Price) from `order` o, orderitem i where o.OrderId=i.OrderId and o.tableid is NULL and status!=5 group by o.OrderId");
...
?>
	<tr>
		<td>
			<?php echo $row["MIN(CreatedTime)"]; ?>
		</td>
		<td>
			<?php echo $row["SUM(Quantity)"]; ?>
		</td>
		<td>
			<?php echo $row["SUM(Quantity*Price)"]; ?>
		</td>
		<td>
			<a href="editorders.php?id=<?php echo $row['OrderId']?>">Edit</a>
		</td>
		<td>
			<a href="bill.php?orderid=<?php echo $row['OrderId']; ?>" target="_blank" > Checkout </a>
		</td>
	</tr>
<?php		
...
<td>
<input type="button" onClick="addRow()" value="Add to order" style="width: 110;"/>
</td>
</tr>
<tr>
	<td>
