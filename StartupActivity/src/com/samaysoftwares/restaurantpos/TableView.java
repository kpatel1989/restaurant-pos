...
public class TableView extends Activity implements OnTabChangeListener {
	...
	class CheckOut extends AsyncTask<String, String, String>{
		...
		protected String doInBackground(String... params) {
			...
			try {
				String orderidSP = prefs.getString("orderid", "-2").trim();
				response = httpclient.execute(new HttpGet("http://"+ info.getIP()+ "/testTreeView1/android_checkout.php?checkout=01&orderid="+ orderidSP.trim()));
				...
				if (statusLine.getStatusCode() == HttpStatus.SC_OK) {
					...
					String url = "http://"+info.getIP()+"/testtreeview1/android_getbill.php";
					url += "?" + paramString;
	...
	class CallWaiter extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... args) {
			...
			try {
				...
				String url = "http://"+ info.getIP()+ "/testTreeView1/android_waiterrequests.php?waiterrequests=1&devicename="+ devicename;
				System.out.println(url);
				response = httpclient.execute(new HttpGet("http://"+ info.getIP()+ "/testTreeView1/android_waiterrequests.php?waiterrequests=1&devicename="+ devicename));
				StatusLine statusLine = response.getStatusLine();
				if (statusLine.getStatusCode() == HttpStatus.SC_OK) {
		...
		protected String doInBackground(String... params) {
			...
			try {
				...
				while(!responseString.equals("9") && !responseString.equals("2")){
					...
					System.out.println("http://"+ info.getIP()+ "/testTreeView1/android_checkstatus.php?devicename="+ info.getDevice());
					response = httpclient.execute(new HttpGet("http://"+ info.getIP()+ "/testTreeView1/android_checkstatus.php?devicename="+ info.getDevice()));
					//System.out.println("http://" + info.getIP()+ "/testTreeView1/android_order.php?devicename="+ info.getDevice() + "&" + orderQty + "&"+ orderidSP.trim() + "&" + userid);
					StatusLine statusLine = response.getStatusLine();
					if (statusLine.getStatusCode() == HttpStatus.SC_OK) {
		...
		protected String doInBackground(String... params) {
			...
			if (orderCount <= 0) {
				...
				try {
					...
					response = httpclient.execute(new HttpGet("http://"+ info.getIP()+ "/testTreeView1/android_order.php?devicename="+ info.getDevice() + "&" + orderQty + "&"+ orderidSP.trim() + "&" + userid));
					System.out.println("http://" + info.getIP()+ "/testTreeView1/android_order.php?devicename="+ info.getDevice() + "&" + orderQty + "&"+ orderidSP.trim() + "&" + userid);
					StatusLine statusLine = response.getStatusLine();
					if (statusLine.getStatusCode() == HttpStatus.SC_OK) {
						ByteArrayOutputStream out = new ByteArrayOutputStream();
